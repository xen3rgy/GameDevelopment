...
import {PARKING_SPACES,TRANSIT_STOPS,stopById} from './mobility-layout.js?v=0.8.1';
...
export class MobilityScene{
 constructor(world,kit){this.world=world;this.root=new THREE.Group();world.scene.add(this.root);const {box,cylinder,sign}=kit;
  for(const p of PARKING_SPACES.filter(p=>!p.service)){const y=groundHeight(p.x,p.z)+.012;for(const dz of [-1.15,1.15])box(this.root,p.x,y,p.z+dz,6.3,.016,.065,0xd0c9ac);for(const dx of [-3.15,3.15])box(this.root,p.x+dx,y,p.z,.065,.016,2.3,0xd0c9ac);}

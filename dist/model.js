...
export class GameModel{
 ...
 get wealth(){return this.s.money+this.s.bank-this.s.debt+Object.entries(this.s.businesses).reduce((v,[id])=>v+BUSINESSES[id].cost,0)+this.s.properties*150000+ownedVehicles(this.s).reduce((n,v)=>n+resale(v),0)}
 ...
 basketAdd(id){
  ...
  const nearShelf=Object.values(SHOP_POINTS).some(p=>p.items?.includes(id)&&inReach(this.s.position,p,'indoor'));if(!nearShelf)return false;
 ...
 checkout(){
  const s=this.s,point=SHOP_POINTS.checkout;if(s.interior!=='shop'||!inReach(s.position,point,'indoor')||!s.basket.length)return false;
 ...
 work(action,index=0){
  ...
  const near=p=>p&&Math.hypot(s.position.x-p.x,s.position.z-p.z)<=(j.type==='courier'?3:2.2);
  if(action==='deliver'&&j.type==='courier'){
   ...
   const p=WORK.cleaning[index];j.cleaned??=Array.from({length:j.progress},(_,i)=>i);if(!Number.isInteger(index)||!p||j.cleaned.includes(index)||!near({x:p[0],z:p[1]}))return false;j.cleaned.push(index);j.progress++;s.needs.hygiene=clamp(s.needs.hygiene-2);s.needs.energy=clamp(s.needs.energy-1);
 ...
 this.onChange()}
 daily(){insuranceDay(this);let s=this.s;finishCafe(s,'Mitternacht');s.collected=[];let states=['Normal','Normal','Aufschwung','Rezession'];s.economy=states[Math.floor(s.day/3)%4];s.weather=s.day%3===0?'rain':'clear';let income=0,cashAdjustment=0;
 ...
 buyVehicle(id,offer=null){return purchaseVehicle(this,id,offer)}
 serviceVehicle(action){return fleetCommand(this,action,this.s.vehicle?.uid)}
...
export function validateSave(raw){
 ...
 if(!['story','sandbox'].includes(s.mode)||s.day<1||!Number.isSafeInteger(s.day)||s.minute>=1440||s.quest>QUESTS.length||s.properties>10000||!Number.isInteger(s.quest)||!Number.isSafeInteger(s.properties)||!Number.isSafeInteger(s.rentDue)||!Number.isSafeInteger(s.xp)||!Number.isSafeInteger(s.reputation))throw Error('Ungültiger Fortschritt.');
 ...
 for(const b of Object.values(s.businesses)){for(let k of ['stock','staff','price','quality','profit','sales','costs'])if(!Number.isFinite(b[k]))throw Error('Ungültige Unternehmensdaten.');if(!Number.isSafeInteger(b.stock)||!Number.isSafeInteger(b.staff)||!Number.isSafeInteger(b.quality)||typeof b.open!=='boolean'||typeof b.marketing!=='boolean'||b.stock<0||b.stock>7||b.staff<0||b.staff>3||b.price<.8||b.price>1.4||b.quality<1||b.quality>3)throw Error('Ungültige Unternehmenswerte.')}

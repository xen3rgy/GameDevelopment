import {WorkshopScene} from './workshop-scene.js?v=0.7.2';
...
export class World{
 ...
 makeOwnedVehicle(id){if(this.vehicleMesh)this.scene.remove(this.vehicleMesh);let g=new THREE.Group();if(id==='bike'){for(let z of [-.7,.7]){let wheel=new THREE.Mesh(new THREE.TorusGeometry(.4,.045,8,24),mat(0x252d30));wheel.rotation.y=Math.PI/2;wheel.position.set(0,.46,z);g.add(wheel);cylinder(g,0,.45,z,.035,.8,0xb6bdb8)}box(g,0,.73,0,.1,.16,1.25,0x5d9292);box(g,0,1.05,0,.08,.55,.1,0x5d9292);box(g,0,1.3,-.1,.35,.12,.42,0x293435);box(g,0,1.4,.68,.65,.08,.1,0xb3b6a9);box(g,0,1.04,.65,.08,.68,.08,0x566a6a)}else{g=createCar(artKit,id,id==='sport'?0x984f3b:id==='van'?0xb6bdba:0x447c84)}this.vehicleMesh=g;this.vehicleId=id;this.scene.add(g)}
 ...
 const key=JSON.stringify(s.drops);if(key!==this.lastDrops){for(let d of this.drops)this.scene.remove(d.mesh);this.drops=s.drops.map((d,i)=>{let mesh=d.id==='bottle'?this.bottle(d.x,d.z):box(this.scene,d.x,groundHeight(d.x,d.z)+.25,d.z,.45,.5,.45,0xb99b64);return{...d,index:i,mesh}});this.lastDrops=key}

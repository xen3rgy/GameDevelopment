import {VEHICLES,ITEMS,LOCATIONS} from './data.js?v=0.8.0';
import {inventoryWeight,transferItem} from './inventory.js?v=0.8.0';
import {exteriorContains} from './city-layout.js?v=0.8.0';
import {PARKING_SPACES,parkingById} from './mobility-layout.js?v=0.8.0';
import {inReach} from './interactions.js?v=0.8.0';
import {overlaps,vehicleBody} from './traffic.js?v=0.8.0';
...
export function purchaseVehicle(model,id,offerKey=null){
 const s=model.s;initFleet(s);if(s.job?.fieldAction||s.dailyLife?.action||s.workshop?.active?.action||!atGarage(s)||!Object.hasOwn(VEHICLES,id)||s.fleet.length>=FLEET_LIMIT)return false;
...
export function fleetCommand(model,action,uid,findExit=null){
 const s=model.s;initFleet(s);const v=s.fleet.find(v=>v.uid===uid);if(!v||s.riding||s.transit||s.job?.fieldAction||s.dailyLife?.action||s.workshop?.active?.action)return false;
 ...
 if(action==='fuel'){if(v.id==='bike')return false;const price=Math.ceil((100-v.fuel)*30);if(!model.spend(price))return false;v.fuel=100;return true;}
 if(action==='repair'){const price=Math.max(0,Math.ceil((100-v.condition)*(v.id==='bike'?10:120))-Math.floor(v.claimCredit));if(!model.spend(price))return false;v.condition=100;v.claimCredit=0;return true;}
 return false;
}
export function transferTrunk(model,uid,direction,index,count){
 const s=model.s,v=ownedVehicles(s).find(v=>v.uid===uid);if(!v||v.stored||s.inside||s.riding||s.transit||s.dailyLife?.action||s.job?.interaction||s.job?.fieldAction||s.workshop?.active?.action||!inReach(s.position,trunkPoint(v),'trunk'))return false;
...
export function vehicleTravel(v,metres,impactSpeed=0){
 ...
 if(impactSpeed>3){const damage=Math.min(v.condition,(impactSpeed-3)*.65);v.condition-=damage;if(v.insured)v.claimCredit=Math.min(12000,(v.claimCredit||0)+damage*120*.7);}
...
export function parkVehicle(model,canStand,canPark){
 const s=model.s,v=s.vehicle;if(!s.riding||!v||Math.abs(v.speed)>.5||s.transit)return false;

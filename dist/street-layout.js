import {ROADS,ROAD_X,ROAD_Z} from './city-layout.js?v=0.7.2';
export const ROAD_HEIGHT=.14, PAVEMENT_HEIGHT=.30, HALF_ROAD=6.5;
...
export const onRoad=(x,z)=>ROADS.some(p=>inRect(p,x,z));
...
export const RAMPS=CROSSINGS.flatMap(c=>[-1,1].map(side=>({x:c.x+(c.axis==='x'?side*7.5:0),z:c.z+(c.axis==='z'?side*7.5:0),w:c.axis==='z'?5.2:2,d:c.axis==='z'?2:5.2,side,axis:c.axis,crossing:c})));
export function rampHeight(r,x,z){
 const normal=(r.axis==='z'?z-r.crossing.z:x-r.crossing.x)*r.side-HALF_ROAD;
 const lateral=Math.abs(r.axis==='z'?x-r.x:z-r.z);
 const t=Math.max(0,Math.min(1,normal/2)),wing=Math.max(0,Math.min(1,lateral-1.6));
...
export function streetPatches(){
 ...
 for(const r of RAMPS){const set=r.axis==='z'?xs:zs,c=r.axis==='z'?r.x:r.z;set.add(c-1.6);set.add(c+1.6);}

...
test('new and old saves preserve western positions, parked vehicles, dropped items and household contents',()=>{
 ...
 m.add('cheese');m.drop(0);const loaded=validateSave(JSON.parse(JSON.stringify(m.s)));assert.deepEqual(loaded.storage,m.s.storage);assert.deepEqual(loaded.fridge,m.s.fridge);assert.equal(loaded.vehicle.id,m.s.vehicle.id);assert.equal(loaded.vehicle.x,m.s.vehicle.x);assert.equal(loaded.fleet.length,1);

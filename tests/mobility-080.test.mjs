...
test('insurance pays daily, wear remains gradual, accident credit cannot pay cash and sale requires empty trunk',()=>{
 const m=garage();m.buyVehicle('car');const v=m.s.vehicle,before=m.s.money;assert.ok(fleetCommand(m,'insurance',v.uid));assert.equal(m.s.money,before-250);vehicleTravel(v,1000,8);assert.equal(v.odometer,1);assert.ok(v.condition<100&&v.condition>90);assert.ok(v.claimCredit>0);const damage=v.condition,credit=v.claimCredit;assert.equal(fleetCommand(m,'repair',v.uid),false);assert.equal(v.condition,damage);assert.equal(v.claimCredit,credit);v.trunk=[{id:'water',count:1}];assert.equal(fleetCommand(m,'sell',v.uid),false);m.s.money=0;m.advance(1440-m.s.minute);assert.equal(v.insured,false);
});
